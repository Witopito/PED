...
void TABBCalendario::insertarOrdenado(TCalendario c) {
	if (c < raiz->item) {
		if (raiz->iz.EsVacio()) {
			TNodoABB *x = new TNodoABB();
			x->item = c;
...
bool TABBCalendario::buscaCalendario(const TCalendario &c, TABBCalendario sub) {
	bool auxiz, auxde;
...
void TABBCalendario::Copiar(const TABBCalendario &origen) {
	if (origen.raiz != NULL) {
...
bool TABBCalendario::auxBuscar(TABBCalendario a) {
...
bool TABBCalendario::operator==(const TABBCalendario &a) {
	bool auxiz = true, auxde = true;
	//TABBCalendario aux(*this);
...
void TABBCalendario::InordenAux(TVectorCalendario &v, int &a) {
	if (!EsVacio()) {
...
void TABBCalendario::PostordenAux(TVectorCalendario &v, int &a) {
	if (!EsVacio()) {
...
void TABBCalendario::PreordenAux(TVectorCalendario &v, int &a) {
	if (!EsVacio()) {
		v[a] = raiz->item;
		a++;
...
TABBCalendario TABBCalendario::operator+(TABBCalendario &b) {
	TVectorCalendario v;
	v = b.Inorden();
